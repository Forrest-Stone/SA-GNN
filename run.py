...
logging.basicConfig(level=logging.DEBUG,
                    filename='sagnn-test.log', filemode='a')
...
def evaluation(magnn, train, test_set, topk=20):
    ...
    for batchID in range(num_batches):
        ...
        # 测试的时候这个地方内存过大报错
        # with torch.no_grad():
        rating_pred = magnn(batch_test_sequences,
...
def get_slice(inputs_np, for_pre=False):
    ...
    for u_input in inputs:
        ...
        if for_pre:
            u_A = np.array([[0, 1/2, 1/2], [1/2, 0, 1/2], [1/2, 1/2, 0]])
        else:
            u_A = np.array([[0, 1/2, 1/2, 0, 0, 0], [1/2, 0, 1/2, 0, 0, 0], [1/2, 1/2, 0, 0, 0, 0], [
                           0, 0, 0, 0, 1/2, 1/2], [0, 0, 0, 1/2, 0, 1/2], [0, 0, 0, 1/2, 1/2, 0]])
        # # u_A = np.zeros((max_n_node, max_n_node))
        # u_A = np.eye(max_n_node)
        # if for_pre:
        ...
        #         u_A[i][i + 1] = 1
        # else:
        #     for i in np.arange(len(u_input) // 2 - 1):
        #         u_A[i][i + 1] = 1
        #         if i < len(u_input) // 2 - 2:
        #             u_A[i][i + 2] = 1
        #     # for i in np.arange(len(u_input)//2, len(u_input)-1):
        #     #     u_A[i][i+1] = 1
        #     #     if i < len(u_input)-2:
        #     #         u_A[i][i+2] = 1
        #     for i in np.arange(len(u_input) - 1, len(u_input) // 2, -1):
        #         u_A[i][i - 1] = 1
        #         if i > len(u_input) // 2 + 1:
        #             u_A[i][i - 2] = 1
        A.append(u_A)
    return A
...
def train_model(train_data, test_data, config):
    ...
    magnn = MAGNN(num_users, num_items, config, device).to(device)
    ...
    for epoch_num in range(config.n_iter):
        ...
        for batchID in range(num_batches):
            ...
            prediction_score = magnn(
                batch_sequences, batch_users, items_to_predict, gnn_np, False)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--d1', type=int, default=100,
                        help='item embedding size')
    parser.add_argument('--d2', type=int, default=50,
