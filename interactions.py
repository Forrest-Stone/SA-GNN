...
class Interactions(object):
    ...
    def to_sequence(self, sequence_length=5, target_length=1):
        ...
        num_subsequences = sum(
            [c - max_sequence_length + 1 if c >= max_sequence_length else 1 for c in counts])

        sequences = np.zeros((num_subsequences, sequence_length-target_length),
                             dtype=np.int64)
        ...
        test_sequences = np.zeros((self.num_users, (sequence_length-target_length)//2),
                                  dtype=np.int64)
        ...
        for i, (uid, item_seq) in enumerate(_generate_sequences(user_ids,
                                                                ...
                                                                max_sequence_length)):
            if uid != _uid:
                test_sequences[uid][:] = item_seq[-((
                    sequence_length-target_length)//2):]
                test_users[uid] = uid
                _uid = uid
            # sequences_targets[i][:] = item_seq[-target_length:]
            # sequences[i][:] = item_seq[:sequence_length-target_length]
            sequences_targets[i][:] = item_seq[(
                sequence_length-target_length)//2:(sequence_length-target_length)//2+target_length]
            # sequences[i][:] = item_seq[:sequence_length // 2].extend(item_seq[sequence_length // 2 + 1:sequence_length])
            # sequences[i][:] += item_seq[sequence_length // 2 + 1:sequence_length]
            sequences[i][:] = np.delete(item_seq[:sequence_length], np.s_[(
                sequence_length-target_length)//2:(sequence_length-target_length)//2+target_length], axis=0)
